...
def discover_pbip_path() -> str | None:
    """Find the report definition path of the .pbip file open in Power BI Desktop.

    Reads the PBIDesktop.exe command line via WMI to extract the .pbip path,
    then derives the <Name>.Report/definition folder from it.
    Returns the definition folder path string, or None if not found.
    """
    if platform.system() != "Windows":
        return None
    ...
    try:
        result = subprocess.run(
            [
                ...
                "(Get-WmiObject Win32_Process -Filter \"Name='PBIDesktop.exe'\").CommandLine",
            ],
            capture_output=True,
            text=True,
            timeout=8,
        ...
        match = re.search(r'"([^"]+\.pbip)"', cmd_line, re.IGNORECASE)
        if not match:
            return None
        pbip_path = Path(match.group(1))
        definition = pbip_path.parent / f"{pbip_path.stem}.Report" / "definition"
